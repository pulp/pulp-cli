...
then
  ...
  rsync -avzh --delete-delay -f '+ en/index.html' -f'P en/*' site/ doc_builder_pulp_cli@docs.pulpproject.org:/var/www/docs.pulpproject.org/pulp_cli/
else
  [ "${REF_TYPE}" = "tags" ]
  # publish to docs.pulpproject.org/pulp_cli/en/{release x.y}
