...
CERT=$(cat "${CERTFILE}")
KEY=$(cat "${KEYFILE}")
expect_succ pulp file remote create --remote "cli_test_file_remote" --url "$FILE_REMOTE_URL" --client-cert @"$CERTFILE" --client-key @"$KEYFILE" --ca-cert @"$CERTFILE"
expect_succ pulp file remote destroy --remote "cli_test_file_remote"

expect_succ pulp file remote create --remote "cli_test_file_remote" --url "$FILE_REMOTE_URL" --client-cert "$CERT" --client-key "$KEY" --ca-cert "$CERT"
expect_succ pulp file remote destroy --remote "cli_test_file_remote"
