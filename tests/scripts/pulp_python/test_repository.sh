...
cleanup() {
  ...
  pulp python remote destroy --name "cli_test_python_remote1" || true
  pulp python remote destroy --name "cli_test_python_remote2" || true
}
trap cleanup EXIT

REMOTE1_HREF="$(pulp python remote create --name "cli_test_python_remote1" --url "http://a/" | jq -r '.pulp_href')"
REMOTE2_HREF="$(pulp python remote create --name "cli_test_python_remote2" --url "http://b/" | jq -r '.pulp_href')"
